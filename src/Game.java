...
public class Game {
	...
	private List<String> themes = Arrays.asList("Animais", "Celebridades", "Cinema", "Comida", "Desenho", "Futebol",
			"Heróis", "Música", "Saude", "Video Games");
	...
	private void updatePlayerScore() {
		this.getActualRound().getPuns().forEach((integer, pun) -> {
			this.players.stream().filter(player -> player.getId() == pun.getPlayerId()).findFirst()
					.ifPresent(player -> {
	...
	private void setMaxPontuation(List<Pun> punList) {
		...
		if (punList.get(0) != null)
			scoreboardMaxPontuation = punList.get(0).getPontuation();
	...
	private void showScoreboard() {
		broadcast("\nPLACAR - RODADA " + this.actualRound.getNumber() + "\n");
		...
		Integer timeLeft = 5;
		while (timeLeft > 0) {
			broadcast("\nRodada iniciando em " + timeLeft + " segundos...");
			try {
				Thread.sleep(1000);
			} catch (InterruptedException e) {
				e.printStackTrace();
	...
	private void broadcastPoll(String message, UUID playerUUID) {
		...
		List<Player> otherPlayers = this.players.stream().filter(player -> player.getId() != playerUUID)
